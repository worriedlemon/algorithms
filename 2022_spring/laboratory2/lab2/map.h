...
// A bundle consist of an assosiative pair of [name] and [value]
...
struct Bundle
{
	...
	Bundle(T1 name, T2 value) : name(name), value(value) {}
	Bundle(T1 name, T2* value) : name(name)
	{
		this->value = *value;
	}
	~Bundle() {}

	friend bool operator==(Bundle<T1, T2>& b1, Bundle<T1, T2>& b2)
	{
		return (b1.name == b2.name && b1.value == b2.value);
	}
	...
	friend ostream& operator<<(ostream& stream, Bundle<T1, T2> bundle)
	{
		stream << "{" << bundle.name << ", " << bundle.value << "}";
		return stream;
	}
};

// Map based on self-balancing Red & Black Tree structure
...
public:
	...
	{
		return *((TreeNode*)&FindTreeNode(key))->value;
	}

	friend ostream& operator<<(ostream& stream, Map<T1, T2>& rbtree)
	{
	...
	T2& Find(T1 key)
	{
		TreeNode* current = &FindTreeNode(key);
	...
	void Insert(T1 key, T2* value)
	{
		...
		{
			delete root;
			root = new TreeNode(new T1(key), value);
			root->SwitchColor(NodeColor::Black);
		}
		else
		{
			...
			TreeNode* newNode = new TreeNode(new T1(key), value, current->parent);
			delete current;
			current = newNode;
			if (key > *(current->parent->key)) current->parent->right = current;
			else current->parent->left = current;
	...
	void Remove(T1 key)
	{
		...
		else
		{
			...
			y->SwitchColor(nodeToBeDeleted->color);
		}
	...
	List<Bundle<T1, T2>>* GetKeysAndValues()
	{
		List<TreeNode*>* list = DFT();
		List<Bundle<T1, T2>>* result = new List<Bundle<T1, T2>>;
		while (list->head != nullptr)
		{
			result->PushBack(Bundle<T1, T2>(T1(*((TreeNode*)list->head->data)->key), T2(*((TreeNode*)list->head->data)->value)));
			list->PopFront();
		}
		return result;
	}
...
// Map, which contains a pair of [key] as a custom type and [value] as a non-negative number (count)
template <typename T> using CountingMap = Map<T, size_t>;
#endif // RBTREE