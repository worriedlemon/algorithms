...
// Finding some new 
double FindFlowToAdd(FlowNetwork& graph, size_t currentVertex, size_t drain, bool* visitedVertexes, double lowestCapacity = INF)
{
    double** capacities = graph.GetValues();
    ...
    for (size_t processedVertex = 0; processedVertex <= drain; processedVertex++)
    {
        ...
        if (!visitedVertexes[processedVertex] && capacities[currentVertex][processedVertex] > 0)
        {
            ...
            double foundFlow = FindFlowToAdd(graph, processedVertex, drain, visitedVertexes, lowestCapacity);
